...
public class Sword extends Weapon {
	...
	public Sword(String name, Level level, float swingSpeed, float bladeDamage, float handleDamage,
		...
		for(int i = 0; i < handleLength/handlePartSize; i++){
			parts.add(new SwordPart(handlePartSize, handleDamage, handleColor));
		}
		for(int i = 0; i < bladeLength/bladePartSize; i++){
			parts.add(new SwordPart(bladePartSize, bladeDamage, bladeColor));
		}
		direction = new Vector3(0, 0, 0);
		this.swingSpeed = swingSpeed;
		delay = 0;
		particleDelay = bladePartSize/swingSpeed;
	}
	...
	public void update(Level level, Player player, Controls controls) {
		...
		swordDir.scale(level.getDeltaTime()*swingSpeed);
		...
		if(swordDir.getLength() != 0){
			...
			for(int i =0; i < parts.size(); i++){
				...
				if(delay <=0){
					level.getParticles().createParticle(false, 0.1f, part.getBoundingBox().getX(), swordPos.getX(),swordPos.getY(), swordPos.getZ(), 
